...
- 万不得已在 worktree 内切过主干，**merge 完第一时间 `git checkout -` 切回隔离分支归还**，绝不停在主干上（见步骤 4c）。

## 步骤 2：项目 setup

自动探测并装依赖（有哪个装哪个）：`package.json`→装包（按 lockfile 选包管理器：`pnpm-lock.yaml`→pnpm / `yarn.lock`→yarn / `package-lock.json`→npm / `bun.lock` / `bun.lockb`→bun） / `Cargo.toml`→build / `requirements.txt`/`pyproject.toml`→装 / `go.mod`→download / `pom.xml`→依赖解析 等。
...
- **分支 checkout 错位**：在 worktree 内 `git checkout master/main` 做 merge 或图省事 → 主干的唯一 checkout 位被挪进临时 worktree，主仓和其他会话全切不到主干，干完不归还就一直卡死。→ worktree 钉死隔离分支，要合主干去主仓 `git -C "<主仓>" merge`，切过务必 `git checkout -` 归还（步骤 1.6 / 4c）。
...
- **用补丁提取绕过分支**：AI 容易把 worktree 改动 diff/apply 到主仓，或 cherry-pick 单个提交。→ 优先让 `$BRANCH` 正常回流；合并形态按项目约定，不明确就问（步骤 4c）。
- **硬编码 base/路径**：`main...HEAD` 或 `.worktrees/$BRANCH` 会扫错 / 删错。→ 用 `$BASE_REF`、`$WT` 和 `git -C "<主仓路径>"`（步骤 4a/4c）。
- **撞号但 git 不报**：两条分支各占同一个 Flyway 序号 / proto tag / 错误码等 → git 不报冲突，合进去行为错乱。→ 合并前扫非文本语义冲突，本分支改号（步骤 4b）。
