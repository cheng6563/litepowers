...
能可靠推断的直接采用，**只询问答案会改变执行路线且无法从现有事实确定的事项**。存在这类未决事项时，使用平台的结构化交互工具（Claude Code 中为 `AskUserQuestion`）给出 2–4 个真实选项、简述影响并标出推荐项，然后等待用户回答；不要用普通文本提问后继续执行。平台没有结构化交互能力时才改用普通文本提问，并同样停下等待。用户已经明确的决定不重复确认；明确、局部、可逆且无外部副作用的任务，内部确定合同后直接执行，不展示问卷。
